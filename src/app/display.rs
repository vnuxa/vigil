...
use cosmic::{iced::{alignment::{Horizontal, Vertical}, Border, Color, Font, Length, Pixels, Point, Radius, Rectangle, Shadow, Size}, iced_core::{layout, renderer::Quad, text::{LineHeight, Shaping, Wrapping}, Text}, iced_wgpu::graphics::text::cosmic_text::{fontdb::{self, Database, FaceInfo, Query, Source}, Font as ExtraFont, Stretch, Style, Weight}, widget::Widget};
...
#[derive(Clone)]
pub struct TerminalDisplay<const NUM_ROW: usize, const NUM_COLUMN: usize>{
...
impl<const NUM_ROW: usize, const NUM_COLUMN: usize> TerminalDisplay<NUM_ROW, NUM_COLUMN> {
    pub fn new(font_name: String, line_height: f32) -> Self {
        ...
        let font = font_id.first().unwrap_or_else(|| panic!("Could not find font name {}", font_name ));
...
fn get_glyph_size(font_source: Source, index: u32, glyph: char) -> f32 {
    ...
    let hor_advance = face.glyph_hor_advance(face.glyph_index(glyph).unwrap()).unwrap() as f32;
    let upem = face.units_per_em() as f32;
...
impl<Message, const NUM_ROW: usize, const NUM_COLUMN: usize> Widget<Message, cosmic::Theme, Renderer> for TerminalDisplay<NUM_ROW, NUM_COLUMN>
where
    Message: Clone,
{
        ...
        for (index, row) in self.cells.iter().enumerate() {
            ...
            for bundle in row  {
                ...
                let size = if bundle.character_end == bundle.character_start {
                    Size::new(self.glyph_size * (if bundle_offset == 0 { 1 } else { bundle_offset }) as f32, self.line_height)
                } else {
                    // TODO:  using a workaround for the glyph size with other fonts, dont know how
                    // this will work out with longer texts
                    Size::new(self.glyph_size * (bundle.character_end - bundle.character_start + bundle_offset + 1) as f32, self.line_height)
